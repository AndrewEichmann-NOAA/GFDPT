...
C
	...
	CALL ST_RMBL ( substr (2), namodel (1), lng, ier )       !convert substr(2) to namodel (string) and
...
C  Modified as following:       
       ...
       do n = 1, numfcst
         ...
         CALL ST_RMBL ( substr (1), fcst_ymdhf(n), lng1, ier )
     ...
     +    yyyyfcst(n),mmfcst(n),ddfcst(n),hhfcst(n)
         ...
         CALL ST_RMBL ( substr (2), obsv_ymdhf(n), lng2, ier )
     ...
     +     yyyyobsv(n),mmobsv(n),ddobsv(n),hhobsv(n)
         ...
         do nt=1,4
           ...
           CALL ST_RMBL(substr(2*nt+1),fcst_ymdhf2(nt,n),lng1,ier )
...
c     +                hhfcst2(nt,n),fffcst2(nt,n)
           ...
           CALL ST_RMBL(substr(2*nt+2),obsv_ymdhf2(nt,n),lng2,ier )
...
C-------------------------------------------------------------------------------
C Following are modified by Binbin Zhou to read both variable's name and GRIB id 
C from control file
...
C  Modified as:   
        ...
        if (num.gt.6) then        !has FHO or AFHO
          CALL ST_RMBL( substr(6), fho(1), nchrfho(1), ier)
          
         if(fho(1)(1:1).eq.'F') then    !FHO
          fhomrk(1) = num - 6
          do nx = 7, num 
            CALL ST_RMBL(substr(nx),fhothr(1,nx-6),
     +               nchrfhothr(1,nx-6),ier)
     ...
     +           (nchrfhothr(1,nx-6):nchrfhothr(1,nx-6))
          ...
          afhomrk(1) = num - 6
          do nx = 7, num
            CALL ST_RMBL(substr(nx),afhothr(1,nx-6),
     +               nchrafhothr(1,nx-6),ier)
              if(tendencymrk(1).eq.0) then
        ...
        IF(numvarbl.GE.2) THEN
         DO n=2,numvarbl
           ...
           if (num.gt.5) then
             ...
             CALL ST_RMBL( substr(5), fho(n), nchrfho(n), ier)

            if(fho(n)(1:1).eq.'F') then       !for FHO
            
             fhomrk(n) =  num - 5
             do nx = 6, num
              CALL ST_RMBL(substr(nx),fhothr(n,nx-5),
     +                     nchrfhothr(n,nx-5),ier)
     ...
     +           (nchrfhothr(n,nx-5):nchrfhothr(n,nx-5))
             ...
             afhomrk(n) =  num - 5
             do nx = 6, num
              CALL ST_RMBL(substr(nx),afhothr(n,nx-5),
     +                     nchrafhothr(n,nx-5),ier)
              if(tendencymrk(n).eq.0) then
