...
dd=${date:6}
tmp=`expr $dd + 1`  # next day
dd2=`printf "%02i" $tmp`   # display as 2-digit with 0-pad in front
tmp=`expr $dd - 1`  # previous day
dd0=`printf "%02i" $tmp`   # display as 2-digit with 0-pad in front
...
sed -e "s/D2/$dd2/" tmpFile5 > ${CONF_DIR}/gfs_atm_list_${date}.config
...
sed -e "s/M2/$mm/"  tmpFile3 > tmpFile4
sed -e "s/D0/$dd0/" tmpFile4 > tmpFile5
sed -e "s/D2/$dd2/" tmpFile5 > ${CONF_DIR}/gfs_sfc_list_${date}.config
rm tmpFile tmpFile2 tmpFile3  tmpFile4 tmpFile5
