...
MODULE IO_Misc
  ...
  PUBLIC :: Out4InterpolTest,ReadHdrCldInfo,ReadCldInfo,ReadList,ReadList2,ReadList4,Open_Text_File
...
!--- no modification of base file name add prefix and suffix Eric S. Maddy 2014/04/10
  subroutine ReadList4(iu,ListName,rdrFiles,nFiles,tdrFiles,pathTDR,pref,sufix)
    !---Input/Output variables
    INTEGER                                 :: iu,nFiles
    CHARACTER(LEN=*)                        :: ListName,pathTDR,pref,sufix
    CHARACTER(LEN=*), DIMENSION(:), POINTER :: rdrFiles,tdrFiles
    ...
    DO While (.true.)
       ...
       indx=INDEX(xfile,'/',.true.)
       IF (xfile.eq.'') EXIT
       IF (indx .eq. 0) CALL ErrHandl(ErrorType,Err_AbsolPathNotPresnt,'(in ReadList4)') 
       nFiles=nFiles+1
    ENDDO
100 CONTINUE
    close(iu)
    ...
    DO ifile=1,nFiles
       ...
       indx=INDEX(xfile,'/',.true.)
       tdrFiles(ifile) = trim(adjustl(pathTDR))//trim(adjustl(pref))// &
            trim(adjustl(xfile(indx+1:len_trim(xfile))))//trim(adjustl(sufix))
    ENDDO
    close(iu)
    return
  end subroutine ReadList4
