...
Pro plot_grid,tmp,sfcMask,map_name,minvalue,maxvalue,latmin,latmax,lonmin,lonmax,title,sfcPick,div,format,$
	      color_table_index=color_table_index, order=orderx, qcfail_val=qcfail_valx
...
qcfail_val = -99.0
IF (KEYWORD_SET(qcfail_valx)) THEN qcfail_val = qcfail_valx
...
for irow=0, NROW-1 do begin
  ...
  if tmp(icol,irow) eq qcfail_val  then image(icol,irow) = 252
...
IF (KEYWORD_SET(orderx)) THEN warp = REVERSE(warp,2)
...
write_png, map_name, TVRD(), r, g, b, order=orderx
...
device,set_resolution=2.*[550, 325]
...
;        Date          Programmer      		Description of change
;    ============    ==============    =========================================
;     11/05/2014     Eric S. Maddy     
;===================================================================================================
Pro plot_jacob,tmp,xval,yval,xrange,yrange,xtitle,ytitle,title,map_name,minvalue,maxvalue,Xticks,$
    div,fmt,Yticks,YtickName,x,psfc,color_table,isMirs
...
PRO sectionDisplay,array,tit,leg,fmt,nc,x,y,ArrMin,ArrMax,xtit,ytit,wid,xsize,ysize,chsz,xlines,ylines
  ...
  ;TV,image,100,180
  TV,image,xsize/8,ysize/6
  plot,xx,yy,ytick_get=ytick,xtick_get=xtick,yrange=[min(yy),max(yy)],xrange=[min(xx),max(xx)],/nodata,$
    position=[xsize/8,ysize/6,xsize/8+xsize,ysize/6+ysize],/noerase,charsize=1.2*chsz,thick=5
  ...
  plot,xx,yy,ytickname=yticknew,xtickname=xticknew,yrange=[min(yy),max(yy)],xrange=[min(xx),max(xx)],/nodata,$
    position=[xsize/8,ysize/6,xsize/8+xsize,ysize/6+ysize],$
...
PRO DensityScatter,minX,maxX,minY,maxY,X,Y,xtit,ytit,tit,chsz,iplotStats,stats,img_name,gridScale,symbol_size,ps_out, font_size=font_sizex
  ...
  if (ps_out eq 1) then begin
      ...
      device,filename=img_name+'.eps',/color,bits_per_pixel=8,xsize=xz,ysize=yz,xoffset=1,yoffset=1,/encapsulated,$
...
;  position=[1.6/xz,1.8/yz,12.8/xz,16.2/yz]
  ...
  ;---- write out image file
  if (ps_out eq 0) then write_png, img_name, TVRD(), rr, gg, bb;, /order
  device,/close
END
...
;
;
; Description:  Grids X and Y data, where the 
;               red color represents the highest density of points.
;		Output in encapsulated PS or PNG, scale is log 10.
;
;
; Keywords:     stats=statsx       -> plot bias and standard deviation as
;                                 f(xbins)
;               statspos=statsposx -> set location of statistics legend
; 
; Subroutines needed:
;       - None
;
;
; History:
;       04-16-2014      Eric S. Maddy, RTi @ NOAA/NESDIS/STAR
;
;===============================================================

PRO DensityPlot,minX,maxX,minY,maxY,X,Y,xtit,ytit,tit,chsz,iplotStats,stats,$
                img_name,binsizex,binsizey,symbol_size,ps_out, stats=statsx, statpos=statposx, $
...
;  position=[1.6/xz,1.8/yz,12.8/xz,16.2/yz]
  ...
  nc=nc-2
  ;---- set up grids from scale factor
  gridX=findgen(((maxX-minX)/binsizex)+(1))
  gridY=findgen(((maxY-minY)/binsizey)+(1))
  gridX(*)=0
  gridY(*)=0
  ...
  ny=n_elements(gridY)
  for ix=0,nx-1 do begin
      gridX(ix)=(ix*binsizex)+minX
  endfor
  for iy=0,ny-1 do begin
      gridY(iy)=(iy*binsizey)+minY
  endfor
  ;---- loop over grids and count number of points in each cell
  gridDensity=make_array(nx,ny,/float,value=0)
  colorarray=make_array(nx,ny,/int,value=0)
  ...
  for ix=0,nx-2 do begin
      for iy=0,ny-2 do begin
          ...
          indcol=long(xind) > 1L < nc
          colorarray(ix,iy)=indcol-1
          usersym,[0,0,binsizex,binsizex,0],[0,binsizey,binsizey,0,0],/fill
          oplot,[gridX(ix)],[gridY(iy)],psym=8,color=indcol-1,symsize=symbol_size
      endfor
  endfor
  oplot, [minX, maxX], [0., 0.], THICK=1, LINESTYLE=1

  ;---- compute statistics
  IF (KEYWORD_SET(statsx)) THEN BEGIN
      avgplot = FLTARR(nx) + !VALUES.F_NAN 
      sdvplot = FLTARR(nx) + !VALUES.F_NAN 
      FOR i = 0, nx-2L DO BEGIN
          idx = WHERE(x ge gridx[i] and x lt gridx[i+1],nidx)
          IF (nidx eq 0) THEN continue 
          avgplot[i] = MEAN(y[idx])
          IF (nidx gt 2) THEN sdvplot[i] = SQRT(VARIANCE(y[idx]))
      ENDFOR
      idx = WHERE(x ge gridx[nx-1],nidx)
      IF (nidx ne 0) THEN BEGIN
          avgplot[nx-1] = MEAN(y[idx])  
          IF (nidx gt 2) THEN sdvplot[nx-1] = SQRT(VARIANCE(y[idx]))
      ENDIF
      OPLOT, gridx, SMOOTH(avgplot,5,/NAN), COLOR=250, THICK=3
      OPLOT, gridx, SMOOTH(sdvplot,5,/NAN), COLOR=250, LINESTYLE=2, THICK=3
  ENDIF
  nelts=n_elements(x)
  c10=moment(y)
  rms=sqrt(mean((y)^2))
...
;      plots,[minX,maxX],[minY,maxY]
      comm10=strmid(strcompress('Bias: '+string(c10[0])),0,20)
      comm11=strmid(strcompress('Std Dev: '+string(stdev(y))),0,15)
      comm12=strmid(strcompress('Points: '+string(nelts)),0,15)
      comm15=strmid(strcompress('RMS: '+string(rms)),0,11)
      xs=minX+(maxX-minX)/18.
      r0=minY
      r=maxY
      scl = 0.9
      IF (KEYWORD_SET(statposx)) THEN BEGIN
        CASE statposx OF 
           1: BEGIN
             scl = 0.90
           END
        ...
        ENDCASE
      ENDIF
      xyouts,xs,r0+(scl*(r-r0)),comm10,charsize=chsz*0.7
      xyouts,xs,r0+((scl-0.04)*(r-r0)),comm11,charsize=chsz*0.7
      xyouts,xs,r0+((scl-0.08)*(r-r0)),comm12,charsize=chsz*0.7
      xyouts,xs,r0+((scl-0.12)*(r-r0)),comm15,charsize=chsz*0.7
  endif
  ;---- create colorbar
  position_bar=fltarr(4)
  position_bar[0]=position[0]+0.05
...
PRO DensityPlotContour,minX,maxX,minY,maxY,X,Y,xtit,ytit,tit,chsz,iplotStats,stats,$
...
;  position=[1.6/xz,1.8/yz,12.8/xz,16.2/yz]
  ...
  nc=nc-2
  ;---- set up grids from scale factor
  gridX=findgen(((maxX-minX)/xbinsize)+(1))
  gridY=findgen(((maxY-minY)/ybinsize)+(1))
  gridX(*)=0
  gridY(*)=0
  ...
  ny=n_elements(gridY)
  for ix=0,nx-1 do begin
      gridX(ix)=(ix*xbinsize)+minX
  endfor
  for iy=0,ny-1 do begin
      gridY(iy)=(iy*ybinsize)+minY
  endfor
  ;---- loop over grids and count number of points in each cell
  gridDensity=make_array(nx,ny,/float,value=0)
  gridXa=make_array(nx,ny,/float,value=0)
  gridYa=make_array(nx,ny,/float,value=0)
  colorarray=make_array(nx,ny,/int,value=0)
  for ix=0,nx-2 do begin
      idx=where(X gt gridX(ix) and X le gridX(ix+1),cnt1)
      for iy=0,ny-2 do begin
          gridXa[ix,iy] = gridX[ix]
          gridYa[ix,iy] = gridY[iy]
          if (cnt1 gt 0) then begin
              idy=where(y(idx) gt gridY(iy) and y(idx) le gridY(iy+1),cnt2)
              gridDensity(ix,iy)=cnt2
          endif
  ...
  ;---- plot
  if (ps_out eq 0) then plot,gridX,gridY,$
    /nodata,title=tit,xtitle=xtit,ytitle=ytit,$
    yrange=[minY,maxY],position=position,xrange=[minX,maxX],thick=3
  if (ps_out eq 1) then plot,gridX,gridY,$
    /nodata,title=tit,xtitle=xtit,ytitle=ytit,$
    yrange=[minY,maxY],position=position,xrange=[minX,maxX],thick=3,font=0
  ...
  if (max(gridDensity) gt 100000) then maxGridDensity=100000
  maxGridDensity = 100.
  GridDensity=(GridDensity/max(GridDensity))*100.
  levs=findgen(100)/1.
  cols=levs*2.3

  CONTOUR,GridDensity,GridXa,GridYa,levels=levs,c_colors=cols,xrange=[minX,maxX],xstyle=1,ystyle=1,$
    yrange=[minY,maxY],xtitle=xtit,ytitle=ytit,title=tit,charsize=chsz, position=position, /fill
;;   levs = [25., 50., 75.]
;; ;  cols  = [0.25*
;;   CONTOUR,GridDensity,GridXa,GridYa,levels=levs,c_colors=cols[[24,49,74]],xrange=[minX,maxX],xstyle=1,ystyle=1,$
;;     yrange=[minY,maxY],xtitle=xtit,ytitle=ytit,title=tit,charsize=chsz, position=position, THICK=5, $
;;     /OVERPLOT, C_ANNOTATION=['Q1','Q2','Q3'], C_THICK=5
  ...
  IF (KEYWORD_SET(statsx)) THEN BEGIN
      ...
      FOR i = 0, nx-2L DO BEGIN
          ...
          IF (nidx gt 2) THEN sdvplot[i] = SQRT(VARIANCE(y[idx]))
      ENDFOR
      idx = WHERE(x ge x[nx-1],nidx)
      IF (nidx ne 0) THEN BEGIN
          avgplot[nx-1] = MEAN(y[idx])  
          IF (nidx gt 2) THEN sdvplot[nx-1] = SQRT(VARIANCE(y[idx]))
      ENDIF
      OPLOT, gridx, avgplot, COLOR=250, THICK=3
      OPLOT, gridx, sdvplot, COLOR=250, LINESTYLE=2, THICK=3
  ENDIF
  nelts=n_elements(x)
  c10=moment(y)
  rms=sqrt(mean((y)^2))
...
;      plots,[minX,maxX],[minY,maxY]
  ...
  colorbar3,xticks=0,ncolors=nc,/horizontal,minrange=0.,maxrange=100.,title='Density of Points', $
    charsize=chsz*0.7,position=position_bar,divisions=5
