...
;===============================================================
; Name:       ComputeHistogramStats
;
; Type:       IDL Subroutine
;
; Description: Create histogram of data and compute bias, stdev
;
; Arguments:
;      Name      Dim      Type             Description
;      ---------------------------------------------------
;      -tbd      (nchan)  Input		   Tb difference array
;      -nbin              Input            Number of bins for histogram
...
PRO ComputeHistogramStats, tbd, nbin, nchan, histBias,histStdv

    histBias=fltarr(nchan)
    histStdv=fltarr(nchan)
    ...
    for ichan=0,nchan-1 do begin
        ...
        ytit='Nubmer of Pts'
        dumx=[1,2] & dumy=[1,2]
        PLOT, dumx, dumy, CHARSIZE=1, TITLE='Channel '+strcompress(string(ichan+1)),$
        ...
        r0=ymin
        r=ymax
        xyouts,xs,r0+(0.85*(r-r0)),sfcstr,charsize=0.75
        xyouts,xs,r0+(0.77*(r-r0)),comm1,charsize=0.75
        xyouts,xs,r0+(0.69*(r-r0)),comm2,charsize=0.75
        xyouts,xs,r0+(0.61*(r-r0)),comm3,charsize=0.75
    endfor
