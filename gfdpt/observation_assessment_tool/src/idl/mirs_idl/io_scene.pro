...
;	- EmissAnlvec        I           Emissivity vector
...
PRO WriteSceneFile,iu,iTyp,nLay,nLev,nChan,nAbsorb,nParmCLW,nParmRain,nParmSnow,nParmIce,nParmGrpl,$
                   ...
                   IceLay,GrplLay,Angl,Emiss,EmissAnlvec,Refl,WindSp,Tskin,SfcPress,SfcTyp,qc,lat,lon,node,$
...
; 	- EmissAnlvec         O            Emissivity vector
...
PRO ReadScene,file_in, nprofiles,nlay,nlev,nchan,nAbsorb,nParmCLW,nParmRain,nParmSnow, $
              ...
              Pres_lev,Temp_lay,Abso_lay,clw,rain,snow,ice,graupel,Angle,Emiss,EmissAnlvec,Refl,   $
    ...
    EmissAnlvec = Scene.EmissAnlvec(0:nprofiles-1,*)
...
PRO LoadHdrScene,fileTruth,topId,nProfs2read,iu,iTyp,nPrf,nLay,nLev,nChan ,nAbsorb,$
    ...
    AbsorbID2use4declaration=lonarr(nabsorb) 
    IF (nabsorb eq 2) THEN AbsorbID2use4declaration=[1,3] ELSE $
       AbsorbID2use4declaration = [1,3,5,2,6,4]
    readu,iu,AbsorbID
    cfreq=fltarr(nChan)
    readu,iu,cfreq
    polar=lonarr(nChan)
    ...
    IF (SceneIsNotDefined or NprfBiggerDeclaredSize) THEN BEGIN
        Scene={                                       $
          ...
          AbsorbLayVec: fltarr(nprf,nLay,6),    $
          TPWvec:       fltarr(nPrf),                 $
...
;	- emissanlvec                I/O          Emissivity vector (nchan)
...
PRO LoadCoreScene,iu,iprof,Scene,ProfIndx,pressLay,pressLev,tempLay,Absorbents,xclw, $
                  xrain,xsnow,xice,xGrpl,emiss,emissanlvec,Angl,Windsp,Tskin,SfcPress,SfcTyp,   $
   ...
   IF (Scene.iTyp eq 2) THEN BEGIN
     readu, iu, emiss
     Scene.EmissAnlvec(iprof,0:Scene.nChan-1) = emiss
     readu, iu, emiss
     Scene.ReflVec(iprof,0:Scene.nChan-1) = emiss
  ENDIF
...
PRO LoadSceneFile,fileTruth,topId,Scene,nProfs2read
    ...
    emissanlvec  = fltarr(Scene.nchan)
