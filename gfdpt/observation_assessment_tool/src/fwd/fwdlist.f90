...
!       06-12-2014      Eric S. Maddy, RTI @ NOAA/NESDIS/STAR
!                       - Modified code to read a list of files 
!
!===============================================================

Program ForwardModelList
  !---CRTM module
  USE CRTM_Module
  !---Utility functions
  USE misc
  ...
  INTEGER                                  :: iu_list, nFilesoutput, nFilesinput
  INTEGER                                  :: ifile, nAbsorb  
  CHARACTER(LEN=256), DIMENSION(:),       POINTER     :: GeophInputFiles,OutputFiles
  CHARACTER(LEN=256), DIMENSION(:),       POINTER     :: dumfiles
  ...
  !----------------------------------------------------------------------------
  !   The input and output files are actually lists of files 
  !   Read the file names of input geophysical data and output radiance data 
  !----------------------------------------------------------------------------
  iu_list = 10
  call ReadList(iu_list,trim(CntrlConfig_fwd%GeophInputFile),GeophInputFiles,nFilesinput,dumFiles,'.','')
  DEALLOCATE(dumFiles)
  call ReadList(iu_list,trim(CntrlConfig_fwd%OutputFile),OutputFiles,nFilesoutput,dumFiles,'.','')
  DEALLOCATE(dumFiles)
  CLOSE(iu_list)
  IF (nFilesinput .ne. nFilesOutput) stop
  ...
  !-----------------------------------------------------
  IF (CntrlConfig_fwd%SensorID < sensor_ID_IR) THEN ! ESM 10/17/2014
    nAbsorb=2   ! only for MW instruments
  ELSE
    nAbsorb=6 
  ENDIF
  CALL CRTM_Atmosphere_Create(Atmos,MAXLAYS,nabsorb,MAXCLOUDS,MAXAEROSOL)
...
!       Sfc_K,Options,RTSolution,RTSolution_K)
  ...
  FilesLoop: DO ifile=1,nfilesInput
     
    !----------------------------------------------------------------------------
    !   Open geophysical data file and load header
    !----------------------------------------------------------------------------
    CALL ReadHdrScene(fID,GeophInputFiles(ifile),Scene,nPrf)
    !---Determine the number of profiles to be processed
    !CALL GetNprofs2Process_fwd(CntrlConfig_fwd%nprofs2process,nPrf,nprofiles)
    !Comment out GetNprofs2Process_fwd and set nProfiles to nPrf 
    !So QCChecking module is not needed -KJG-
    nProfiles=nPrf
    !----------------------------------------------------------------------------
    ...
    ALLOCATE(qc(nqc))
    qc(1:nqc) = 0
    CALL WriteHdrMeasurmts(OutputFiles(ifile),FileOut,nProfiles,nqc,nchan,     &
    ...
    ProfLoop: DO iprof=1,nProfiles
      ...
      IF (CntrlConfig_fwd%iPrintMonitor.eq.1) &
          WRITE(*,'(a10,i6,2i4,a7,40f10.3)')'Prof/Typ:',iprof,qc(1),Scene%iTypSfc,'   TBs:',TB(1:12)
    ENDDO ProfLoop
    !---Close Profiles File and Output File 
    CLOSE(fID)
    CLOSE(FileOut)
    !---Deallocate memory allocated inside FilesLoop
