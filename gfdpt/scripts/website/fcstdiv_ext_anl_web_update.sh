...
export DATADIR=/home/people/emc/www/htdocs/GFDPT/aeichmann/sandbox/main/extdiff/$PDY
...
ls $DATADIR/*_$PDY*.gif | sed -e "s/\/home\/people\/emc\/www\/htdocs\/GFDPT\/aeichmann\/sandbox\/main\/extdiff\/$PDY\///g" -e "s/_hgt_/ hgt /g" -e "s/_tmp_/ tmp /g" -e "s/.gif/ gif/g" -e "s/${PDY}_/${PDY} /g" > $OUTDIR/giflist
...
#            scp $OUTDIR/case_high_$id1.php sdm@ncointra:$WEBDIR
            cp $OUTDIR/case_high_$id1.php $WEBDIR
         fi
      done
   else
      priorityval=low
...
#            scp $OUTDIR/case_low_$id1.php sdm@ncointra:$WEBDIR
            cp $OUTDIR/case_low_$id1.php $WEBDIR
         fi
      done
   fi
done
...
cp $OUTDIR/extanldiff_tab.php $WEBDIR/.
