...
MODULE utils
  ...
  PUBLIC :: interprofile,LayersICWC,AdjustEmiss,interpolprof,interprofilenval,LayersHydro
...
!    print *, nscposRad, nscposBias, rposFactor, iscPos, indPos
    indPos = 1
    !---Calibrate
...
!
! History:
!       04-02-2014      Eric S. Maddy RTi @ NOAA/NESDIS/JCSDA
!
!===============================================================

  SUBROUTINE interprofilenval(XProfIn,PressIn,nLevOut,PressOut,PresBot,PresTop,XProfOut,qc)
    IMPLICIT NONE
    !---I/O variables   
    INTEGER,                 INTENT(IN)   :: nLevOut
    REAL,    DIMENSION(:),   INTENT(IN)   :: XProfIn
    ...
    !---Determine number of vertical levels with good data 
    nVal_Levs = COUNT(XProfIn(:) .ge. -999.)
    IF (nVal_Levs .lt. 2) THEN
       XProfOut(:) = -999.
       qc(1) = 1
       RETURN
    ...
    IF (nVal_Levs .ge. 2) THEN
       !---Create index of array elements with good data, these are the elements to interpolate
       ALLOCATE (idx_inLevs(nVal_Levs),XProfTrun(nVal_Levs),PresProfTrun(nVal_Levs))
       idx_inLevs      = PACK( (/(i,i=1,SIZE(XProfIn(:)))/), (XProfIn(:) .ge. -999.))
       PresTop         = PressIn(idx_inLevs(1))
       PresBot         = PressIn(idx_inLevs(nVal_Levs))
       XProfTrun(:)    = XProfIn(idx_inLevs)
       PresProfTrun(:) = PressIn(idx_inLevs)
    ...
    ENDIF
    RETURN
  END SUBROUTINE interprofilenval
...
!
!       07-02-2014      Eric S. Maddy:
!                       -Added option for GPM highest frequency channels
!===============================================================
  ...
  SUBROUTINE AdjustEmiss(EmissIn,EmissOut,nChan,cFreq,sensor_id)
    ...
    IF (sensor_id .eq. sensor_id_gpm) THEN
       EmissOut(10) = EmissOut(8) - random_noise(ichan)
       EmissOut(11) = EmissOut(9) - random_noise(ichan)
       EmissOut(12) = EmissOut(8) - random_noise(ichan)
       EmissOut(13) = EmissOut(9) - random_noise(ichan)
    ENDIF
    IF (sensor_id .eq. sensor_id_f18) THEN
       ...
       IF (Em(10) .lt. 0) EmissOut(10) = EmissOut(9)-random_noise(10)
       IF (Em(11) .lt. 0) EmissOut(11) = EmissOut(9)-random_noise(11)
       IF (Em(12) .lt. 0) EmissOut(12) = EmissOut(1)-random_noise(12)
       DO ichan=19,24
          IF (Em(ichan) .lt. 0) EmissOut(ichan) = Em(1)-random_noise(ichan)
       ENDDO
    ENDIF
    IF (sensor_id .eq. sensor_id_airs) THEN
       DO ichan=1,2378
          IF (Em(ichan) .lt. 0) EmissOut(ichan) = EmissOut(1)-random_noise(ichan)
       ENDDO
    ENDIF
