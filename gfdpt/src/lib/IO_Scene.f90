...
! 10-15-2014    E.S. Maddy RTi @ NOAA/NESDIS/JCSDA
!               added trace species molecules for IR simulations
!
!-----------------------------------------------------------------------------------------------
...
MODULE IO_Scene
  ...
  TYPE   :: Scene_type
    ...
    INTEGER                                    :: iCO2       !Index of CO2 within absorb vector
    INTEGER                                    :: iCO        !Index of CO within absorb vector
    INTEGER                                    :: iCH4       !Index of CH4 within absorb vector
    INTEGER                                    :: iN2O       !Index of N2O within absorb vector
    !---Positioning Data
    REAL                                       :: lat        !Latitude
    REAL                                       :: lon        !Longitude
    ...
    REAL,              DIMENSION(:),   POINTER :: EmissAnlytc!Emissivities vector analytic
  ...
  INTEGER, PARAMETER :: H2OID = 1, CO2ID=2, O3ID=3, N2OID=4, COID=5, CH4ID=6
...
!===============================================================
  SUBROUTINE setSceneEDRs2default(Scene)
    ...
    Scene%EmissAnlytc(:)  = DEFAULT_VALUE_REAL
...
!===============================================================
  ...
  SUBROUTINE AffectScene(SceneIn,SceneOut)
    ...
    SceneOut%iCO                           = SceneIn%iCO
    SceneOut%iCO2                          = SceneIn%iCO2
    SceneOut%iN2O                          = SceneIn%iN2O
    SceneOut%iCH4                          = SceneIn%iCH4
    ...
    SceneOut%EmissAnlytc(1:SceneOut%nchan) = SceneIn%EmissAnlytc(1:SceneIn%nchan)
...
!===============================================================
  ...
  SUBROUTINE AdjustHdrScene(Scene_Ext,Scene_ExtAdjusted,CovNlay,&
    ...
    ALLOCATE(Scene_ExtAdjusted%AbsorbID(Scene_Ext%nAbsorb), &
         ...
         Scene_ExtAdjusted%EmissAnlytc(Scene_Ext%nchan), &
    ...
    Scene_ExtAdjusted%iCO                           = Scene_Ext%iCO
    Scene_ExtAdjusted%iCO2                          = Scene_Ext%iCO2
    Scene_ExtAdjusted%iN2O                          = Scene_Ext%iN2O
    Scene_ExtAdjusted%iCH4                          = Scene_Ext%iCH4
...
!===============================================================
  ...
  SUBROUTINE PrintScene(Scene)
    ...
    print *, 'EmissAnlytc:',Scene%EmissAnlytc
...
!===============================================================
  ...
  SUBROUTINE InitHdrScene(nLev,nLay,nChan,CentrFreq,polarity,pres_lev,&
    ...
    ALLOCATE(Scene%AbsorbID(Scene%Nabsorb),Scene%pres_lev(nLev),      &
         ...
         Scene%polarity(nChan),Scene%Emiss(nchan),&
         Scene%EmissAnlytc(nchan),Scene%Refl(nchan),  &
...
!===============================================================
  ...
  SUBROUTINE ReadHdrScene(iu,InputFile,Scene,nprf)
    ...
    READ(iu)  Scene%nqc
    !---Find the indexes for H2O and O3 gases (and other trace gas values)
    Scene%iH2O = FindIndx(Scene%AbsorbID,H2OID)
    Scene%iO3  = FindIndx(Scene%AbsorbID,O3ID)
    Scene%iCO  = FindIndx(Scene%AbsorbID,COID)
    Scene%iCO2 = FindIndx(Scene%AbsorbID,CO2ID)
    Scene%iCH4 = FindIndx(Scene%AbsorbID,CH4ID)
    Scene%iN2O  = FindIndx(Scene%AbsorbID,N2OID)
    ...
    ALLOCATE(Scene%pres_lay(1:Scene%nLay),Scene%Temp_lay(1:Scene%nLay), &
         ...
         Scene%EmissAnlytc(1:Scene%nchan),   &
...
!===============================================================
  ...
  SUBROUTINE WriteScene(iu,Scene)
    ...
    !---In case the scene is a diagnostic type 
    ! -- also CRTMs ocean model FastemX requires Reflection passed 
    IF (Scene%iTyp .eq. 2) THEN
       WRITE(iu) Scene%EmissAnlytc(1:Scene%nchan)
       WRITE(iu) Scene%Refl(1:Scene%nchan)
    ENDIF
...
!===============================================================
  ...
  SUBROUTINE ReadScene(iu,Scene,ierr)
    ...
    !---In case the scene is a diagnostic one (need analytic emissivity and reflectivity for CRTM)
    IF (Scene%iTyp .eq. 2) THEN
       READ(iu,err=20) Scene%EmissAnlytc(1:Scene%nchan)
       READ(iu,err=20) Scene%Refl(1:Scene%nchan)
    ENDIF
    !---In case the scene is a retrieved one
    IF (Scene%iTyp .eq. 1) THEN
