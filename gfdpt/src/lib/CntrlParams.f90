...
!       11-06-2014  Eric S. Maddy RTi @ NOAA/NESDIS/JCSDA
!                   added Jacobian type 
!
!-----------------------------------------------------------------------------------------------
...
MODULE CntrlParams
  ...
  PUBLIC :: LoadCntrlParams,LoadCntrlParams_Fwd,LoadCntrlParams_Jacobian
  !----Publicly available data/types
  PUBLIC :: CntrlConfig,CntrlConfig_fwd,CntrlConfig_Jacobian,CntrlConfig_type,CntrlConfig_fwd_type, &
       CntrlConfig_Jacobian_type
  !----Global section of the module
  INTEGER, PARAMETER :: nLEN=256,mxCh=5000,mxEDR=100,mxAtt=2
  ...
  !------This structure relates to the Jacobian version of the forward model code-------------
  TYPE :: CntrlConfig_Jacobian_type
    CHARACTER(LEN=nLEN)                   :: GeophInputFile     !geophysical data file
    INTEGER                               :: SensorID           !Sensor ID
    CHARACTER(LEN=nLEN)                   :: Coeff_Path         !CRTM-based Coeff files path 
    CHARACTER(LEN=nLEN)                   :: InstrConfigFile    !Instrument configuration file
    ...
    CHARACTER(LEN=nLEN)                   :: NoiseFile          !Noise data file (NEDT)
    CHARACTER(LEN=nLEN)                   :: LogFile            !Log file containing the warning/errors
    CHARACTER(LEN=nLEN)                   :: JacobianFile       !Output Jacobian file
    INTEGER                               :: nprofs2process     !Number of profiles requested to process
    INTEGER,             DIMENSION(mxCh)  :: ChanSel            !Selected Channels index vector 
    INTEGER                               :: nChan              !Number of channels
    INTEGER                               :: LEN                !Maximum dimension of the file names (strings)
    ...
    INTEGER                               :: iCldOffOrOn        !Flag to set hydrometeor profiles to 0 for simulation
    INTEGER                               :: iPrintMonitor      !Flag to print on screen a summary of the exec
  END TYPE CntrlConfig_Jacobian_type
  
  TYPE(CntrlConfig_type)     :: CntrlConfig
  TYPE(CntrlConfig_fwd_type) :: CntrlConfig_fwd
  TYPE(CntrlConfig_Jacobian_type) :: CntrlConfig_Jacobian
  
  SAVE CntrlConfig
  SAVE CntrlConfig_fwd
  SAVE CntrlConfig_Jacobian
...
!===============================================================
! Name:        LoadCntrlParams_Jacobian
!
!
! Type:        Subroutine
!
!
! Description:  Load the control parameters used by the fwd and K-matrix model code
!               These are stored in the CntrlConfig_Jacobian structure and 
!               made available in the PUBLIC section of this module.
!
!
! Arguments: None
...
!===============================================================
  ...
  SUBROUTINE LoadCntrlParams_Jacobian()
    INTEGER :: nchan,ichan
    NAMELIST /ContrlP_Jacobian/CntrlConfig_Jacobian
    CntrlConfig_Jacobian%ChanSel = DEFAULT_VALUE_INT
    READ(*,NML=ContrlP_Jacobian)
    CntrlConfig_Jacobian%LEN     = nLEN
    CntrlConfig_Jacobian%mxChan  = mxCh
    !---Determination # of channels (from the channels selection vector)
    nChan =0
    ChanLoop: Do ichan=1,mxCh
       IF (CntrlConfig_Jacobian%ChanSel(ichan).eq.DEFAULT_VALUE_INT) CYCLE  ChanLoop
       nChan=nChan+1          
    ENDDO ChanLoop
    CntrlConfig_Jacobian%nChan = nChan
  END SUBROUTINE LoadCntrlParams_Jacobian
