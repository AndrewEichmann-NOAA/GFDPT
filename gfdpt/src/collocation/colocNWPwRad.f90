...
!       07-24-2014      Eric S. Maddy          Added GFS levels 10mb-70mb to interpolation
!       10-08-2014      Eric S. Maddy          Test for SENSOR_ID_IR for surface emissivity 
!                                              bias corrections
!       10-09-2014      Eric S. Maddy          Added O3 from ECMWF to output (GFS is stub)
!
!===============================================================
...
program colocNWPwRad
  ...
  INTEGER            :: nvarSfc,nvarAtm,nAnalys2use,nLay,nLev,sfcTypeIdx, nlayo3, nlevo3
  ...
  REAL(fp_kind),               DIMENSION(:), ALLOCATABLE   :: Emissivity, EmissAnlytc
  REAL(fp_kind),               DIMENSION(:), ALLOCATABLE   :: Reflectivity
  ...
  REAL               :: xalt,xcover
  REAL               :: PresBotT,PresTopT,PresTopQ,PresBotQ,PresBotO3,PresTopO3
  ...
  REAL,     DIMENSION(:),   ALLOCATABLE :: clwc,ciwc,o3mr
  REAL,     DIMENSION(:),   ALLOCATABLE :: temper2Use,relHum2Use !,clwc2Use
  REAL,     DIMENSION(:),   ALLOCATABLE :: level_p,level_p2Use, level_pO3
  REAL,     DIMENSION(:),   ALLOCATABLE :: layer_t,layer_q,layer_o3,layer_rh,layer_sh
...
!  IF (nfilesNWPatm .lt. 1) CALL ErrHandl(ErrorType,Err_NoFilesFound,'(atm-NWP files in coloc)') 
  ...
  !-----------------------------------------------------
  nAbsorb=2   ! only for MW instruments
  IF (sensor_ID < sensor_ID_IR) THEN ! ESM 10/17/2014
    ALLOCATE(AbsorbID(nAbsorb),qc(nqc))
    AbsorbID = (/1,3/)
  ELSE
    nAbsorb=6 
    ALLOCATE(AbsorbID(nAbsorb),qc(nqc))
    AbsorbID = (/1,3,5,2,6,4/)
  ENDIF
  print *, 'nAbsorb=', nAbsorb
  !---Create Atmophere, Surface and Options structures
  CALL CRTM_Atmosphere_Create(Atmos(1),MAXLAYS,nabsorb,MAXCLOUDS,MAXAEROSOL)
  ...
  ALLOCATE(level_p(nLev),temper(nLev),relHum(nLev),height(nLev),specHum(nLev), &
       clwc(nLev),ciwc(nLev),o3mr(nLev),level_pO3(nlevO3))
  ALLOCATE(sfcArr(nlat,nlon,nvarSfc,nAnalys2use),atmArr(nlat,nlon,nvarAtm,nAnalys2use),&
       ...
       layer_t(nLayOut),layer_q(nLayOut),layer_O3(nLayOut),layer_rh(nLayOut),layer_sh(nLayOut),&
  ...
  do i=1,ntime
      ...
      ELSE IF (nwp_source .eq. 3) THEN
        ...
        level_po3=(/10., 20., 30., 50., 70., 100./)
        
!        level_p=(/100., 150., 200., 250.,&
!                  300., 350., 400., 450., 500., 550., 600., 650.,&
!                  700., 750., 800., 850., 900., 925., 950., 975., 1000./)
  ...
  IF (sensor_ID < sensor_ID_IR) THEN ! ESM 10/08/2014
  ...
  FilesLoop: DO ifile=1,nfile
      ...
      ALLOCATE(emissivity(Ym%nchan),reflectivity(Ym%nchan),&
           EmissAnlytc(Ym%nchan),Emiss2Adjust(Ym%nChan),Emiss_Adjusted(Ym%nChan),tb(Ym%nChan))
...
!        print *, 'Ym%qc(1) = ', Ym%qc(1)
!        Ym%qc(1) = 0
!        print *, Ym%lat, Ym%lon, Ym%Year, Ym%Julday, Ym%secs
        IF ( Ym%qc(1) .eq. 0 ) THEN
           ...
           ELSE IF (nwp_source .eq. 2) THEN
              ...
              DO ilev=1,nLev
                 ...
                 CALL interpolBetw4points(Ym%lat,Ym%lon,Ym%Year,month,day,Ym%secs,atmArr,ilev+4*nLev+1,&
                      sfcTypeIdx,ilat,ilon,itime,nlat,nlon,latNWP,lonNWP,timeNWP,o3mr(iLev),qc,dHourNWP)
              ...
              ENDDO
              !---convert from mass mixing ratio to ppmv
              o3mr(1:nLev) = 1.e3*o3mr(1:nLev)*(MW_DRYAIR/MW_O3)
!              print *, 'o3mr=',o3mr(1:nlev)
              ...
              CALL intrpLevs2Lays(o3mr,level_p,nLevOut,pressLevOut,PresBotO3,PresTopO3,pressLayOut,layer_o3,qc)
...
!              DO ilev=1,nlevo3-1
!                 CALL interpolBetw4points(Ym%lat,Ym%lon,Ym%Year,month,day,Ym%secs,atmArr,&
!                      52+ilev,sfcTypeIdx,ilat,ilon,itime,nlat,nlon,latNWP,lonNWP,timeNWP,o3mr(ilev),qc,dHourNWP)
!                 o3mr(ilev) = o3mr(ilev)*1.e3
!              ENDDO
              !---what to do here?
!              DO ilev=nlevo3, nlev-1
!                 o3mr(ilev) = 0.20*o3mr(nlevo3-1)
!              ENDDO
!              o3mr(1:nLev) = o3mr(1:nLev)*(MW_DRYAIR/MW_O3)
!              print *, o3mr(1:nlev)
!              stop
              ...
              DO i=1,nLayOut
                 ...
                 IF (layer_rh(i) .gt. 0 .and. layer_t(i) .gt. 0) THEN
                    layer_q(i) = RelHum_to_mixingratio(layer_rh(i)/100.,layer_t(i),PressLayOut(i))
                 ENDIF
                 layer_o3(i) = layer_o3(i)*1000.0    !ESM kg/kg*(1000g/kg) = g/kg (modified units in FwdOperator.f90)
              ENDDO
              layer_o3(:) = 0.0
          ...
          ! windSp = wind_random * 30.0
           ...
           ELSE
             sfcClass = 0 
             IF (isfcTyp.ne.0) sfcClass=1
!             IF (isfcTypCRTM.ne.0) sfcClass = 1
           ENDIF 
...
!           print *, 'h2o=',layer_q(1:nlayOut)
!           print *, 'temp=',layer_t(1:nlayOut)
!           print *, 'iprof   = ', iprof
!           print *, 'nchan   = ', Ym%nchan
!           print *, 'Ym%angle= ', Ym%angle(1:Ym%nchan)
!           print *, 'angle   = ',mean(Ym%angle(1:Ym%nchan))
           if (nwp_source /= 2) then 
             Scene%Absorb_lay(1:nLayOut,Scene%iO3)  = 0.                   ! O3
           else
             where(layer_o3(1:nLayOut) .lt. 0)
                layer_o3 =0.
             endwhere
             Scene%Absorb_lay(1:nLayOut,Scene%iO3)  = layer_o3(1:nLayOut)  ! O3 - ESM 10/09/2014
             
           endif
           !---if IR Instrument - nAbsorber = 6
           IF (sensor_ID >= sensor_ID_IR) THEN 
              !---Fill trace gas values for CO, CO2, CH4, N2O
              ! itypeCO = 2-AFGL_MOPITT
              ! itypeCO2= 1-AIRS v6 linear ramp from Globalview 2002-2010 (NOAA-ESRL)
              ! itypeCH4= 0-AFGL CH4 
              ! itypeN2O= 1-AIRS v6 RTA (SARTA v108) reference value 
!              CALL FILL_TRACE(Scene, 2, 1, &
!                   0, 1)
              CALL FILL_TRACE(Scene, CO_DEFAULTTYPE, CO2_DEFAULTTYPE, &
                   CH4_DEFAULTTYPE, N2O_DEFAULTTYPE)
!              print *, 'comr=',Scene%Absorb_lay(1:nLayOut,Scene%iCO)
!              print *, 'co2mr=',Scene%Absorb_lay(1:nLayOut,Scene%iCO2)
!              print *, 'ch4mr=',Scene%Absorb_lay(1:nLayOut,Scene%ich4)
!              print *, 'n2omr=',Scene%Absorb_lay(1:nLayOut,Scene%in2o)
           ENDIF
           ...
           Scene%WindU                   = WindU
           Scene%WindV                   = WindV
           Scene%WindDir                 = 180.*ATAN2(-WindU,-WindV)/3.1415927
           ...
           IF (sensor_ID < sensor_ID_IR) THEN 
             ...
             Reflectivity(1:Ym%nchan)=-999.
             EmissAnlytc(1:Ym%nchan)=-999.
             ...
             call ComputeEmiss(Scene,Ym,tb,Atmos,Sfc,Options,RTSolution,ChannelInfo,MWwaterC,Emissivity,&
                  EmissAnlytc,Reflectivity,LogFile)
             !---Adjust emissivity values for non-ocean
             Emiss2Adjust(1:Ym%nChan) = EmissAnlytc(1:Ym%nChan) 
             IF (SfcClass .eq. 0) Emiss_Adjusted(1:Ym%nChan) = Emissivity(1:Ym%nChan)
             IF (SfcClass .ne. 0) then 
                do ichan = 1, Ym%nChan
                   IF (Emiss2Adjust(iChan) .ge. 1.0) Emiss2Adjust(iChan) = 0.9
                   IF (Emiss2Adjust(iChan) .le. 0.0) Emiss2Adjust(iChan) = 0.1
                enddo
                call AdjustEmiss(Emiss2Adjust,Emiss_Adjusted,Ym%nChan,Ym%centrfreq,sensor_id)
             ENDIF
             !---Write Emissivity and Reflectivity values to scene
             IF (SfcClass .eq. 0) THEN 
                Scene%Emiss(1:Ym%nChan) = Emissivity(1:Ym%nChan)
                !---Reflectivity from FastemX model
                Scene%Refl(1:Ym%nChan)  = Reflectivity(1:Ym%nChan)
             ELSE  !---adjusted emissivity values over land
                Scene%Emiss(1:Ym%nChan) = Emiss_Adjusted(1:Ym%nChan)
                Scene%Refl(1:Ym%nChan)  = 1.-Emiss_Adjusted(1:Ym%nChan)
             ENDIF
             IF(sensor_id .eq. SENSOR_ID_MTSA .and. SfcClass .ne. 0) THEN 
                Scene%Emiss(1:Ym%nChan)= 0.8
                Scene%Refl(1:Ym%nChan) = 0.2
             ENDIF
             !---analytic emissivity everywhere
             Scene%EmissAnlytc(1:Ym%nChan) = EmissAnlytc(1:Ym%nChan)
             
!             IF (SfcClass .ne. 0) Scene%Refl(1:Ym%nChan) = Emiss_Adjusted(1:Ym%nChan)
!             Scene%Ym(1:Ym%nChan)   = Scene%Refl(1:Ym%nCHan)
!             print *, SfcClass
!             print *, '2adjust=', Emiss2Adjust(10:13)
!             print *, 'anlytc =',EmissAnlytc(10:13)
!             print *, 'adjustd=',Emiss_Adjusted(10:13)
!             print *, Scene%Ym(10:13)
!             print *, 'finalo =',Scene%Emiss(10:13)
             IF (ALL(Scene%Emiss .gt. 1) .or. ALL(Scene%Emiss .lt. 0)) THEN
                Scene%Emiss=-999.
                Scene%Refl=-999.
             ENDIF
             
           ELSE ! ESM 10/08/2014 IR sensors - set emissivity = 0. -> force CRTM to compute Emissivity
!             EmissAnlytc(1:Ym%nchan)=-999.
             Scene%Emiss(1:Ym%nChan) = 0.0
             Scene%Refl(1:Ym%nChan)  = 1.
!             print *, Scene%Angle
             call ComputeEmiss_IR(Scene,Ym,Ym%tb,Atmos,Sfc,Options,&
                  RTSolution,ChannelInfo,MWwaterC,Emissivity,LogFile)
             !---Write Emissivity and Reflectivity values to scene
             Scene%Emiss(1:Ym%nChan) = Emissivity(1:Ym%nChan)
        ...
        !---Output results
        !-------------------------------------------
!        print *, 'iO3=', Scene%iO3, 'iH2O=',Scene%iH2O
!!        print *, 'O3=',Scene%Absorb_lay(1:nLayOut,Scene%iO3)
!        print *, 'H2O=',Scene%Absorb_lay(1:nLayOut,Scene%iH2O)
!        print *, 'REFL=',Scene%Refl(1:Ym%nChan)
!        print *, Scene%QC(1:nQC)
...
!      DEALLOCATE(emissivity,freq,Emiss2Adjust,Emiss_Adjusted,tb)
      DEALLOCATE(emissivity,emissanlytc,Reflectivity,Emiss2Adjust,Emiss_Adjusted,tb)
  ...
  DEALLOCATE(sfcArr,atmArr,timeNWP,latNWP,lonNWP,sfcArr0,atmArr0,&
       ...
       level_clwc,level_ciwc,level_rain,level_po3)
...
!    if (itime .ge. ntime ) print *, jday0, hour0, jday, hour, nhour, ntime
! for GPM GMI data 2014-08-10 last file
!!$         221          18         223           0          30           6
!!$         221          18         223           0          30           6
!!$         221          18         223           0          30           6
!!$         221          18         223           0          30           6
